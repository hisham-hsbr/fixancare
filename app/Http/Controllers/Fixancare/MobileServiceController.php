...
class MobileServiceController extends Controller
{
    ...
    public function mobileServicesGet()
    {
        ...
        ->editColumn('pendingStatus', function (MobileService $mobile_service) {
            $pendingStatus=($mobile_service->jobStatus->name);
            // $status="Pending";

            $delivered='<span style="background-color: green;color: white;padding: 3px;width:100px;border-radius: 8px;">Delivered</span>';
            $other='<span style="background-color: red;color: white;padding: 3px;width:100px;border-radius: 8px;">Pending</span>';
            // $delivered='<span style="background-color: red;color: white;padding: 3px;width:100px;border-radius: 8px;">Delivered</span>';
            // $other='<span style="background-color: orange;color: white;padding: 3px;padding-left: 5px;padding-right: 5px;width:100px;border-radius: 8px;">' . $status .'</span>';
        ...
        ->addColumn('viewLink', function (MobileService $mobile_service) {

            $viewLink ='<a href="'. route('mobile-services.show', $mobile_service->id) .'" class="ml-2"><i class="fa-solid fa fa-eye"></i></a>';
               return $viewLink;
        })


       ->rawColumns(['jobStatus','pendingStatus','jobType','workStatus','mobileModel', 'mobileComplaint','status','editLink','pdfLink','deleteLink','viewLink'])
        ->toJson();
    }
    ...
    {
        $mobile_service = MobileService::find($id);
        $mobile_models = MobileModel::all();
        $job_types = JobType::all();
        $job_statuses = JobStatus::all();
        $work_statuses = WorkStatus::all();
        $mobile_complaints = MobileComplaint::all();
        return view('back_end.fixancare.mobile_services.show',compact('mobile_service','mobile_models','job_types','job_statuses','work_statuses','mobile_complaints'));
    }
